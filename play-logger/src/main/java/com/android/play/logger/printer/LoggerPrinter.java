package com.android.play.logger.printer;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;

import com.android.play.logger.LoggerUtil;
import com.android.play.logger.adapter.LogAdapter;
...
public class LoggerPrinter implements Printer {
     ...
     */
    private static final int JSON_INDENT_SPACE_NUMBER = 2;
    ...
    public void json(@Nullable String json) {
        ...
        try {
            ...
            if (json.startsWith("{")) {
                ...
                String message = jsonObject.toString(JSON_INDENT_SPACE_NUMBER);
                d(message);
                return;
            }
            if (json.startsWith("[")) {
                JSONArray jsonArray = new JSONArray(json);
                String message = jsonArray.toString(JSON_INDENT_SPACE_NUMBER);
                d(message);
                return;
            }
            e("Invalid Json");
    ...
    public synchronized void log(int priority,
        ...
        if (throwable != null && message != null) {
            message += " : " + LoggerUtil.getStackTraceString(throwable);
        }
        if (throwable != null && message == null) {
            message = LoggerUtil.getStackTraceString(throwable);
        }
        if (LoggerUtil.isEmpty(message)) {
    ...
    public void addAdapter(@NonNull LogAdapter adapter) {
        logAdapters.add(LoggerUtil.checkNotNull(adapter));
    }
